# ☁️ HS-Cloud: Kolla-Ansible 기반 프라이빗 클라우드 자동화 플랫폼
...
2. **프로비저닝 및 구성 자동화** : OpenStack SDK로 자원 생성을 코드화하고 인스턴스 생성 후 수동 개입 없이 SSH 접속 환경과 초기 OS 설정이 완료되도록 Ansible 연동
...
* **Automation** : Ansible (Dynamic Inventory), Packer, OpenStack SDK
* **Backend** : FastAPI (Python 3.10), SQLite
* **Monitoring** : Prometheus, Alertmanager, OpenStack Exporter, Node Exporter 
...
- **일괄 생성** : 유저 등록 요청으로 프로젝트, 네트워크, 서브넷, 라우터, 키페어를 의존 순서에 맞게 생성합니다.
- **이벤트 기반 후속 처리** : 인스턴스 생성 직후 백그라운드에서 OS 초기 설정을 실행합니다.
- **주기 탐지** : 일정 주기로 DB 기록과 실제 자원 상태를 비교해 정리 대상을 분류합니다. 이를 바탕으로 수동으로 자원을 삭제할 수 있습니다.

### 1. 전용 테넌트 인프라 자동 구축
* 유저(프로젝트) 등록 시 OpenStack SDK를 통해 프로젝트 단위로 분리된 네트워크, 서브넷, 라우터, 키페어 생성 및 라우터에 외부 게이트웨이와 내부 인터페이스를 연결합니다. (인스턴스 내에서 인터넷 사용 가능)
...
* 노트북이라는 한정된 호스트 자원의 고갈을 방지하기 위해 정리 대상을 주기적으로 파악하는 절차를 만들었습니다.
...
* **모니터링 파이프라인**: `Prometheus` → `Alertmanager` → `Discord Webhook` 체계를 구축하여 호스트 리소스뿐만 아니라 오픈스택 Nova 컴퓨트 서비스의 가동 상태를 감지합니다.
* **장애 감지 지표 (Alert Rules)**:
    * **Resource Saturation** : RAM 사용률 90% 초과 및 루트 디스크 가용량 10% 미만 감지
    * **Service Availability** : 오픈스택 Nova 컴퓨트 서비스 및 모니터링 에이전트(Exporter) 다운 감지
    * **System Health** : CPU 코어 수 대비 시스템 부하 지수(Load Average) 급증 감지
---
- **실시간 장애 인지 및 디스코드 알림**
